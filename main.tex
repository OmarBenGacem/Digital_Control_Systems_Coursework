...
\begin{equation}\label{acctf}
    sX(s)-AX(s)=BU(s)\rightarrow G(s)=\frac{X(s)}{U(s)}=(Is-A)^{-1}B=\input{equations/tf} 
\end{equation}

\paragraph{Transfer Function Stability}
The stability of the transfer function depends on the determinant (and thus the poles) of ($(Is-A)^-1$). To quickly convert from the Lapclace domain to the Z domain, we 
